...
//
// The app always calls the shared public BFF. Sandpack preview, local
// `npm run dev`, and the Jenkins-deployed build all hit the same URL, so
// the Network tab shows real UOI traffic in every environment. The only
// escape hatch is `window.__UOI_API_BASE__`, which a host page can set
// before the bundle loads to point at a different backend (e.g. during
// emergency debugging). No query params, no localStorage.
const API_BASE: string =
  (typeof window !== "undefined" && (window as any).__UOI_API_BASE__) ||
  "https://agent.mck.aidendigital.com";
